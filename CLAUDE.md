...
> **DECISION (2026-05-30, Riccardo): the perfect from-scratch tool, Path A, full commitment.** After the ladder-gap analysis (`docs/REAL_BUILDS_ANALYSIS.md`), Riccardo's call is explicit: *do NOT change what the Theorycrafter is* (no ladder-seeding, ever) and build a **perfect tool that generates REAL, viable, strong builds from scratch — however absurd the effort**. The calc is already PoB-exact (we run PoB headless); the work is **widening the optimiser's search space + modelling every scaling mechanic**, all derived from vendored data, all fitness-gated. **The committed multi-phase roadmap is in `docs/REAL_BUILDS_ANALYSIS.md` §7** (Fase 1 gemme → 2 cluster jewel → 3 rare craftate → 4 meccaniche DoT/ailment → 5 buff/flask → 6 archetipi trigger → 7 ottimizzatore profondo → 8 scala). This is the north star for all Theorycrafter work going forward.
...
- **FAQ + Privacy/legal pages** (wishlist #5) — two static bilingual pages: `/faq` (`FaqPage.tsx`, 10-question accordion: what FOB is, data sources, PoB-exact DPS, why builds differ from the ladder, pricing freshness, PoB import, cold start, donations, GGG disclaimer) and `/privacy` (`PrivacyPage.tsx`, 6 sections: no personal data, localStorage-only prefs, what a search sends where, donations via PayPal, GGG trademark, open source). Low-prominence nav links at the navbar bottom next to Patch Notes.
...
1. **Atziri's Reflection — Dex requirement not met.** The uniques pass picked a Dex buckler on a non-Dex build (unequippable). Root cause: PoB doesn't enforce attribute requirements in the calc (it only *warns*), so the DPS-eval never saw it. **Fix: an attribute-shortfall penalty in `fitness`** — `shortfall = Σ max(0, Req{Str,Dex,Int} − {Str,Dex,Int})`, `pen *= max(0.2, 1 − shortfall/120)`. A big shortfall (Atziri's ~100 Dex) is hammered to ~0.17 and rejected; the optimiser now picks an equippable off-hand. (A flat-start penalty was tried and reverted — it can't fix the small *inherent* gem-Int shortfalls anyway and destabilises the greedy onto a far worse optimum, e.g. Cyclone 273k→155k.) Residual: 3 builds keep a tiny 2-30 Int shortfall on one level-21 *support* (the main skill is fine) — inherent to the gem's Int req, the player tops it up with one node.
2. **Useless masteries.** `_select_masteries` allocated "30% increased Light Radius" (false-positive via the Energy-Shield keyword in "Light Radius based on ES") and "Minion Defence" on a non-minion build (its "Minions have +8% max Elemental Resistances" read as survival), and the **same effect 2-3×** (illegal — a mastery effect is unique). **Fix: `_mastery_stat_ok`** drops light-radius + minion-on-non-minion stats before scoring; the picker scores **every** effect of each wheel and de-dups on both node AND effect, so two same-type wheels get two *different* effects. Vortex now gets ES/Cold/Spell-Suppression masteries (distinct), Boneshatter Armour/Life/Block (distinct).
3. **Legacy Awakened gems.** The optimiser upgraded damage supports to "Awakened X" — but 3.28 made **every** Awakened damage support Standard-league legacy (only Awakened Empower/Enhance/Enlighten still drop). **Fix: `optimize_awakened` now gates on `_is_available_in_328`** → a no-op in 3.28 (the regular supports are kept). The caster builds lose the legacy ×1.5 (Vortex 1.31M→1.08M, Arc 674k→214k, Spark 357k→157k) — **the honest cost of a current-league-legal build** ("niente fittizio").
4. **"Generated Cluster" name** → a real rare-style name via `gen._rare_name`.

**Final served (PoB-exact, all castable + ascendancy + legal socketing + Ring 2 + Config + imbued + NON-legacy gems + ≤123 pts):** Vortex **1,075,805**, Boneshatter **645,966**, Cyclone **273,258**, Arc **213,762**, Lacerate **208,769**, Spark **156,686**, Ice Shot **61,958**, Blade Vortex **25,393** — all equippable (≤30 Int shortfall on 3, a one-node top-up). Gate: **794 tests / 148 mypy / ruff clean.**

## Step 84 — QA: PoB-import playability fixes (the served builds are now genuinely importable) ✅

Riccardo did a real **PoB import** of a served Theorycrafter build and found **10 concrete bugs** the DPS-only re-eval gate never caught (PoB *warns* about attribute reqs / mana / illegal socketing without zeroing DPS). **All 10 are now fixed.** **The biggest discovery: an importable, castable, ascendancy-allocated build does dramatically more — Vortex 870k → 1.31M, and Boneshatter went from a silent 0 DPS to 695k.** All 8 precomputed builds now re-import into PoB cleanly: castable, ascendancy allocated, legal socketing, real bases, Config tab populated, imbued support where it helps.

The 10 (numbered as Riccardo listed them):
1. **Attributes** — utility gems (guard/warcry) are now attribute-appropriate: Int classes get **Arcane Cloak** + Enduring Cry, Str get **Molten Shell** + Enduring Cry (`_pick_guard`/`_pick_warcry`, `_INT_CLASSES`). Plus a 2nd rare **Ring** (#8) carries `+to all Attributes`.
2. **Mana / castability** — the fitness gate now penalises `ManaUnreserved < ManaCost` (not `< 0`), and a final **`ensure_castable`** guard (in `optimize_build.py`, run last in the precompute) drops auras until `ManaUnreserved ≥ ManaCost`. **Root cause of the worst over-reservation: `_pick_warcry` was handing Int casters `Tempest Shield`, which RESERVES mana** — it over-reserved the low-mana ES builds (Vortex/BV) by ~1900, disabling their auras. Tempest Shield → **Enduring Cry** for every class (zero reservation). Every served build is now castable (re-eval `ManaUnreserved ≥ ManaCost`).
3. **Too many gems per slot** — new **`_assign_gem_groups`** (`pob/encode.py`) is socket-aware: it places the protected groups (primary 6L, multi-aura group, Enlighten/Empower carriers) first keeping all gems, and trims disposable utility groups to the slot's real socket count (Body 6 / Helmet-Gloves-Boots 4 / Weapon 3). No slot is over-socketed.
4. **Imbued support** (3.28 corrupted-gem mechanic) — Riccardo clarified: a corrupted skill gem can carry an *additional* built-in support at **level 1, no socket cost** (e.g. Cyclone + Imbued Critical Damage) — effectively a **7th support** on the main 6L. PoB models it via the `<Skill imbuedSupport="<base name>">` attribute (it resolves `<name>:lower() .. " support"` and adds it as an `ExtraSupport` at level 1 — `CalcSetup.lua`). Shipped: `GemLink.imbued_support` (model + encoder emits the attribute) + **`optimize_imbued`** (tries a damage-support pool on the primary, keeps the PoB-best; fitness-gated). PoB auto-matches it per build — **crit builds → Increased Critical Strikes/Damage, lightning → Hypothermia, DoT/slam → Efficacy; Vortex correctly gets none** (its cold DoT scales on neither crit nor hits). Measured: **Arc +20% / Spark +20% / Lacerate +11% / Boneshatter +9% / Cyclone +8% / Ice Shot +5% / BV +4%.**
5. **Flame Dash counted as damage** — `includeInFullDPS="true"` is now set on the **primary group only** (`orig_i == 0`); every utility/movement/guard group is `false`, so PoB's FullDPS no longer sums a travel skill.
6. **Two travel skills** — the helmet group is now an attribute-aware **guard** (Arcane Cloak / Molten Shell), not a second movement skill, so the build carries exactly **one** travel (Flame Dash for casters / Leap Slam for melee).
7. **"Generated" item names** — generated rares now get a deterministic real-style name (`_rare_name`, seeded by base+slot) instead of "Generated Boots". (The lingering "Generated by FOB" is only the Notes credit line, not an item.)
8. **Unused Ring 2** — `_select_gear` now emits a **2nd Ring**; the encoder labels the two rings `Ring 1`/`Ring 2`; `optimize_uniques` tracks a `used` set so the two rings never get the same unique.
9. **No ascendancy allocated** — new **`_ascendancy_nodes`** (greedy BFS over the ascendancy sub-tree, scored by `_score_text` since `_score_node` returns 0 for ascendancy nodes) allocates ~4 ascendancy notables + their paths within the 8-point lab budget; `_to_pob_tree` now emits them in `<Spec nodes>`. Occultist gets Profane Bloom / Void Beacon / Vile Bastion / Frigid Wake, Jugg gets Unflinching / Untiring / Unbreakable / Unrelenting, etc. **This alone is +41% on Vortex** (the entire ascendancy passive set was previously inert).
10. **Config tab** — the encoder emits a `<Config>` with the build's realistic **player-side** combat state (`conditionUsingFlask` + `conditionKilledRecently`). Per Riccardo's instruction, it does **NOT** touch enemy stats or map modifiers (the enemy stays PoB's default Pinnacle Boss; an enabled curse auto-applies).

**Also fixed (surfaced by the import test): Boneshatter 0 DPS.** PoB disabled the gem with "No usable weapon equipped" — the generator hard-coded **Two Hand Sword** for all melee, but Boneshatter's `weaponTypes` allow only Axe / Mace / Staff / Sceptre (no swords). The melee default is now **Two Hand Axe** (universally accepted — Boneshatter, Cyclone, Lacerate all allow it). `_pick_base`/`_weapon_candidates`/`_build_weapon_group` mirror the change. (The fully-correct fix — pick from each skill's real `weaponTypes` — is a follow-up.) Boneshatter **0 → 635k**.

**Final served builds (PoB-exact, all castable + ascendancy + legal socketing + Ring 2 + Config + imbued support + ≤123 pts):** Vortex **1,314,742** / EHP 61k, Boneshatter **695,092**, Arc **673,538**, Spark **357,176**, Lacerate **296,951**, Cyclone **240,672**, Ice Shot **74,470**, Blade Vortex **33,374**. Gate: **790 tests / 148 mypy / ruff clean**. **All 10 import-bugs fixed — pending only a final user PoB re-import to confirm the warnings are gone.**
...
**Live-generator extension (Empower for spells).** The optimize_links fix only touches the *precompute*; the live generator (`gen._select_supports`, no PoB, for non-matrix requests) didn't get Empower. Added `"Empower"` to `_CORE_SUPPORTS` in the **spell** section (after Elemental Focus) so a live *spell* build picks it ~4th, while an *attack* build stays below the attack supports (ranks it out — correct, +2 levels does little for attacks). Verified: Vortex/Arc/Spark live builds now include Empower; Cyclone/Ice Shot don't. The precompute is unaffected (optimize_links overrides this base pick). Gate: 789 tests / 148 mypy / ruff clean.
...
**Implementation plan (the next focused effort — a major multi-step feature):** (1) **two-skill build model** — a CoC build has a *trigger* attack (Cyclone) + *triggered* spells (Ice Nova + Ice Spear) that are the actual DPS; the generator/optimiser are single-primary-skill and need to carry both. (2) **Gem layout** — 6L = trigger + `Cast On Critical Strike` + 2 triggered spells (via `extra_actives`, already supported) + 2 crit supports (Increased Critical Strikes, Power Charge On Crit). (3) **Crit-aware tree/gear scoring** — the greedy scorer must value crit chance/multi (to reach the trigger-rate cap) **and** the triggered spell's element; add crit to the CoC damage keywords. (4) **Gear** — a fast high-crit weapon, typically **Cospri's Malice** (the iconic CoC unique that triggers spells on crit), + cold/spell + crit; the uniques pass picks it. (5) **Optimiser + matrix + validation** — add a CoC archetype, optimise, and validate the trigger rate is PoB-capped (honest).

**Implementation started + the crux measured (2026-06-03).** Added the CoC encoder scaffolding — `_Encoder(coc_trigger=…)` builds the trigger 6L (Cyclone + `extra_actives`=(Ice Nova) + `_COC_SUPPORTS`=CoC/Increased-Crit/Hypothermia/Cold-Pen), inert until wired into the precompute. **Measured the floor: a CoC Ice Nova / Assassin with the cold tree + a crit claw = only 16k FullDPS** — because the cold-only tree scoring gives **spell** damage but **no attack crit**, so Cyclone barely crits → the trigger rarely fires → Ice Nova is rarely cast. This pinpoints the crux precisely: **the tree scorer must value ATTACK crit chance/multi + attack speed (to reach the ~6.67/s trigger cap) alongside the cold spell damage** — a crit-aware *hybrid* scorer threaded through `optimize_tree` / `_propose_swaps` (which currently propose swaps by the element keywords only, no crit). That hybrid scorer + the gear tuning is the focused next sub-step; shipping the 16k floor would be a weak (fictional-strength) build, so it's deferred to that dedicated effort rather than rushed. Gate: 789 tests / 148 mypy / ruff clean (the scaffolding is inert + lint/type-clean; no precompute change).

**Crit-aware scorer built + measured — CoC is NOT worthwhile for us yet (2026-06-04, focused effort).** Added `_DAMAGE_KEYWORDS["coc_cold"]` (the hybrid crit + attack-speed + cold-spell keyword set) and ran the full CoC pipeline (crit claw + crit-aware tree opt + uniques pass) on a Shadow/Assassin Ice Nova CoC. Measured: tree opt 19k → 19k (the greedy hits a local optimum immediately — the bottleneck isn't the tree), then the **uniques pass lifts it to 65k**. **But 65k is far WEAKER than our regular caster builds (Vortex 870k, Arc 428k, Spark 279k).** The ladder's 84-109M CoC Cyclone relies on mirror-tier gear + perfect crit (reaching the trigger cap) + specific items (Cospri's Malice, mirrored Kalandra's-Touch jewellery) that our from-scratch optimiser doesn't model — and PoB-eval only exposes DPS, not crit-chance/trigger-rate, so diagnosing the trigger bottleneck needs a `pob_eval` extension. **Honest data-driven verdict: CoC produces a *weaker* build than our regular archetypes, so it's not worth serving — the CoC meta is a ladder phenomenon enabled by un-modellable mirror gear.** The `coc_cold` keyword set + the `_Encoder(coc_trigger=)` scaffolding stay (inert, lint/type-clean) as the foundation if the optimiser ever models mirror-tier gear + a crit/trigger-rate diagnostic. Gate: 789 tests / 148 mypy / ruff clean.
...
- **`optimize_flasks` (`scripts/optimize_build.py`).** Forward-selects unique flasks for flask slots 2-5 (the first slot stays a **life/mana base for sustain** — non-negotiable), each kept only if it raises PoB-exact fitness, never repeating a unique. The pool is the vendored uniques DB filtered to the `flask` slot (38 uniques: Bottled Faith, Vessel of Vinktar, The Wise Oak, Replica Rumi's Concoction, Kiara's Determination, Taste of Hate, Lion's Roar, Atziri's Promise, Cinderswallow Urn, Dying Sun, …). Runs after `optimize_uniques`, so the tree / timeless / aura / cluster passes all optimise on the flask-boosted gear. The chosen flasks are reflected into the served display `gear_slots` (the importable `pob_code` already carries them).
- **Honesty.** Flask-active stats are the **standard ladder convention** — every PoB ladder build reports its "flasks up" numbers, and the slots were *already* active in our encode; this step only gives them real flasks. A mirror build sustains near-permanent flask uptime (charges + Pathfinder / enchants / Bottled Faith). The first slot is always kept as a sustain flask.

**Measured (PoB-exact, over Step 77), all reservation-positive + ≤123 pts + importable (re-eval = served) + viability-pass:** **Spark 131.8k → 182.8k (+39%)**, **Arc 191.9k → 266.6k (+39%)**, **Ice Shot 50.4k → 63.6k (+26%)**, **Cyclone 186.7k → 222.1k (+19%)**, **Lacerate 203.1k → 222.8k (+10%)**, **Boneshatter 52.6k → 57.9k (+10%)**, **Blade Vortex 30.8k → 33.8k (+10%)**, **Vortex 368.5k → 391.4k (+6%)** — and the **EHP roughly doubled** on most (Vortex 26.3k → **49.9k**, Cyclone 18.2k → 32.2k, Lacerate 20.0k → 34.5k, Boneshatter 19.0k → 34.1k, Blade Vortex 20.5k → 40.4k) from the defensive flasks (Replica Rumi's block+armour, Kiara's). The optimiser autonomously matched flasks to each build (Lion's Roar on physical melee, Vinktar on lightning, Wise Oak penetration where res is balanced, Bottled Faith everywhere it helps). One cosmetic note: **Spark Fire/Cold res 74** — the optimiser's honest +39% trade kept a 1-under-cap res (viability-flagged; user tops up 2%). Test: `test_builds_use_unique_flasks_keeping_a_sustain_flask`. Gate: 787 tests / 148 mypy / ruff clean.
...
**Negative result — multi-cluster "tree co-design" tried, does NOT beat single-cluster (2026-06-01).** The Step-76 "Honest scope" hoped that *leaning on 2-3 cluster hubs* (allocate fewer regular damage nodes, lean on 2-3 clusters) would beat one cluster. It was implemented (greedy `optimize_clusters` adding up to 3 Large clusters at distinct reachable sockets, with the precompute net-comparing k=0/1/2/3 after trimming to 123 pts) and **measured — it loses**. The PoB-exact diagnostic on Vortex: k=1 **386k** DPS / 25.0k EHP, k=2 392k / 23.2k, k=3 384k / 19.7k — DPS rises, but each extra cluster costs ~14 points that the trim takes from *defensive/res* nodes, so EHP falls and a resistance drops below cap; on the fitness (DPS × viability × EHP) **k=0 or k=1 always wins**. The regular tree's damage notables are simply denser value per point than additional cluster sub-trees. **Reverted** to the Step-77 single-cluster pipeline (committed, no regression). The real ladder-tier cluster win needs a different lever (cluster-specific scaling items like Voices, or Medium clusters with DoT-multi themes for DoT builds), not more Large clusters bolted on — deferred. Don't re-attempt naive multi-cluster.
...
**The mechanism — fully solved end-to-end (proven reloadable, +14% measured):**
- **Allocation + serialization (the breakthrough).** The cluster nodes go in the **`<Spec nodes>` attribute** (raw ids), but the `<Spec>` MUST carry **`clusterHashFormatVersion="2"`** — without it PoB defaults to the legacy v1 hash format and **crashes** on raw cluster ids (this was the blocker behind every failed serialization attempt). With v2, PoB's `ImportFromNodeList` defers unknown ids to `allocSubgraphNodes`, generates the cluster sub-tree from the socketed jewel, then allocates them. **The generated cluster ids are socket-dependent** (socket 55190 → 65696-65708; socket 7960 → 65568-65578), so they're read back per socket (pass 1).
- **Connectivity + budget.** The Large socket must be **reachable from the build's allocated tree** (else the cluster is orphaned + deallocated by `BuildAllDependsAndPaths`) → BFS-path to the nearest size-2 socket (Vortex → socket 7960, 5 hops). A Large cluster costs **~14 passive points** (socket + path + ~8-12 cluster nodes) → reshapes the tree allocation.
- **Proven (reloadable):** Vortex + a Large Spell cluster (Arcane Adept + Doryani's Lesson) at socket 7960, encoded with `clusterHashFormatVersion="2"` + the cluster ids in the attribute → **CombinedDPS 324,991 → 370,227 (×1.14) from one cluster**, and the code re-imports cleanly. A build runs 2-3 large + several medium clusters, so the compound lever is large.

**Implementation plan (the next step, Fase 2 proper — now FULLY de-risked):** (1) vendor the cluster data (themes+enchants per size from `ClusterJewels.lua` + the notable pool with theme-validity from `ModJewelCluster.lua`) → `data/cluster/cluster_3_28.json` + `poe1_fob.gear.clusters`; (2) `encode_pob_code` gains cluster support: emit `clusterHashFormatVersion="2"`, the cluster jewel `<Item>`s + `<Socket>`s, and the cluster node ids in the tree; (3) a **two-pass `optimize_clusters`**: pick a build-relevant theme + notables, BFS-path to a reachable Large socket, encode+eval to read back the generated cluster ids (pass 1), then encode with them allocated (pass 2), fitness-gated; (4) budget-aware — `trim_to_budget` accounts for the ~14 pts/cluster. Gate unchanged: 781 tests / 145 mypy / ruff clean (spike local-only, no committed code).
...
- **`GemLink.skill_level` / `skill_quality` overrides (model + encoder).** `GemLink` gained two optional fields; `_to_pob_gems` uses them for the *active* skill gem (supports/extra-actives unchanged). Default `None` → the existing 20/20. The optimiser's `_Encoder` builds the primary 6L with `skill_level=21, skill_quality=23` (best-version only — the live generator's estimate stays 20/20). `with_primary_supports` now `model_copy`s the primary so the override survives the support forward-select.

**Measured (PoB-exact, over Step 73):** Vortex 307.7k → **346.6k (+13%)**, Arc 178.9k → **191.9k (+7%)**, Spark 124.6k → **131.8k (+6%)**, Lacerate 195.4k → **203.1k (+4%)**, Cyclone **186.7k**, Ice Shot **50.4k**, Blade Vortex **28.4k** (Boneshatter unchanged). All reservation-positive, ≤123 pts, res-capped, importable, viability-pass. **Cumulative over the start of this initiative (Step 72 → 74): Vortex 255.8k → 346.6k (+35%), Arc 139.7k → 191.9k (+37%)** from the two safe gem-depth levers (Awakened + 21/23). Test: `test_gem_link_level_quality_override`. Gate: 781 tests / 145 mypy / ruff clean.

**Fase 1 (gem depth) is essentially done** (Awakened ✅ + 21/23 ✅; alt-quality/Vaal are marginal and deferred). **Next per the roadmap: Fase 2 — cluster jewels** (the biggest remaining tree lever — vendoring cluster-notable data + modelling the cluster jewel item + sub-tree allocation in the optimiser).

## Step 73 — Real-builds initiative: Awakened gems in the optimiser (2026-05-30) ✅

After Riccardo flagged the huge gap vs the ladder (Vortex 2% of a 13M-DPS top build), a ladder comparison (`docs/REAL_BUILDS_ANALYSIS.md`) decomposed it: the **calc is not the problem** (we already run PoB headless — both numbers are PoB-exact), it's **build generation**. The biggest *safe, measured* lever is gem quality: **Awakened damage supports** (~×1.5 on a caster 6L). This step ships it. **User-facing** (precomputed caster DPS +20-28%).

- **Encoder bug fixed (`pob/encode.py` `_skill_id`).** Awakened supports must use PoB's "Plus" convention on the *base* id — "Awakened Controlled Destruction" → **`SupportControlledDestructionPlus`** — NOT the literal `SupportAwakenedControlledDestruction` the old code (and `gems_3_28.json`) produced, which **PoB silently ignores** (the gem does nothing). This is why earlier Awakened attempts would have been no-ops. Verified: the fixed id makes PoB apply the gem.
- **`optimize_awakened` (`scripts/optimize_build.py`).** Upgrades the primary 6L's damage supports to their Awakened versions where PoB-exact fitness rises (Awakened is strictly stronger — higher gem level + an extra effect like Awakened Elemental Focus's penetration). Skips Empower/Enhance/Enlighten (utility). Runs early in the precompute pipeline (after `optimize_links`) so the weapon/uniques/tree passes optimise around the stronger gems. Best-version only — the **live generator keeps the current-league gem allowlist** (Step 45c: 3.28 standard drops only 3 Awakened); the optimiser's precomputed builds are "best version" (Standard-tier, where the full Awakened set exists).

**Measured (PoB-exact, over Step 72):** Arc 139.7k → **178.9k (+28%)**, Spark 99.7k → **124.6k (+25%)**, Vortex 255.8k → **307.7k (+20%)**, Lacerate 177.7k → **195.4k (+10%)**, Ice Shot **49.9k**, Cyclone **183.2k**, Blade Vortex **27.7k** (Boneshatter unchanged — no Awakened raised its fitness). All still reservation-positive, ≤123 pts, res-capped, importable, viability-pass. The caster builds gain most (Awakened Controlled Destruction / Elemental Focus / Spell Echo / Unleash). Test: `test_awakened_support_uses_plus_skill_id`. Gate: 780 tests / 145 mypy / ruff clean.

**Context (the real-builds gap):** Awakened gems are **one** lever (×1.5). The ladder Vortex's 13M is the *product* of many compounding levers we still don't model — cluster jewels, the full crafted-rare DoT-multi suite, Watcher's Eye, deeper tree damage, and (for the meta Cyclone) trigger/Cast-on-Crit archetypes entirely. The remaining safe wins (next): cluster jewels, gem corruption/level (21/23), multi-mod crafted rares. The bigger jump to ladder-tier needs the strategic decision in `docs/REAL_BUILDS_ANALYSIS.md` §4.
...
- **`_forbids_chest(u)` + `_relocate_no_chest(links)` (`scripts/optimize_build.py`).** When the chosen helmet forbids a chest, the primary `GemLink`'s slot moves Body Armour → **Helmet** — the only legal socketing, and it picks up the Bringer's built-in "Socketed Gems are Supported by …" supports. Minimal by design: *only* the primary moves (every other group keeps its slot), so it's a pure overlay on the finished build, not a re-layout.
- **Applied as the FINAL pass** (`precompute_builds.py`, after the trim) — *not* inside the per-slot greedy. Two failed attempts taught why: (a) relocating *during* `optimize_uniques` made the greedy **undervalue** chest-forbidding helmets (dropping the body's defences before the build is built up looks bad → Cyclone wrongly picked Ahn's Contempt, 118k); (b) relocating *before* the tree/aura passes let their greedy re-path to a worse local optimum (Cyclone 143k). As a final overlay on the otherwise-identical Step-69 build, the relocation is the clean, monotonic gain the probe predicted. (Also learned: PoB's `FullDPS` sums *every* `includeInFullDPS` group, so dropping the utility Secondary 4L would have *lowered* FullDPS by ~40k — hence the relocation drops nothing, it only moves the primary.)
- **Display:** the unequippable Body Armour is removed from the served `gear_slots` on chest-forbidding-helmet builds.

**Measured (PoB-exact, over Step 69), all reservation-positive + res-capped + 123 pts:** Lacerate 143.6k → **177.7k (+24%)**, Cyclone 173.3k → **179.4k (+3.5%)** — both now socket the 6L honestly in the Bringer (was a phantom body). Ice Shot **47.3k** (=, Bringer relocation neutral for the bow). Vortex **255.8k** (=, Galesight helmet — body live) and Arc **139.7k** (=, the optimiser picked Fractal Thoughts, an int-stacking helmet → live body + Step-69 gem-level mods, so it never wanted Bringer). Test: `test_chest_forbidding_helmet_sockets_primary_in_helmet`. Gate: 777 tests / 145 mypy / ruff clean.

## Step 69 — Mirror-tier initiative: rare body gem-level influence mods (2026-05-29) ✅

NEXT STEP #3 (multi-mod / influenced rares), first slice. The primary 6L is socketed in the **Body Armour**, so its signature mirror-tier affixes are the gem-level influence mods — **"+1 to Level of Socketed Skill Gems"** (Shaper/Crusader) raises the main skill's level and **"+1 to Level of Socketed Support Gems"** (Elder/Redeemer) raises every support (the classic Awakener's-Orb body). **User-facing.**

- **Data-driven (real RePoE affixes).** `extract_mods.py` keeps `local_socketed_active_skill_gem_level_+` + `local_socketed_support_gem_level_+`; `realmods` resolves the two stems → "+1 to Level of Socketed Skill/Support Gems". These spawn **only via influence** spawn-weight tags (no `default`), so `real_affix_line` augments the body's tags with `_BODY_INFLUENCE_TAGS` (a mirror body IS influenced — honest, not invented). They're ilvl-80 mods → resolve at the **endgame** budget only (cap 86), correctly absent on starter/mid. `_stat_priorities` leads the Body Armour with both. Verified in isolation: PoB applies them (+6% on a live Cyclone).
- **The Bringer of Rain caveat (discovered + diagnosed).** Cyclone/Lacerate/Arc/Ice Shot's optimiser picked **The Bringer of Rain** (helmet), whose mod **"Can't use Chest armour"** makes PoB **ignore the equipped body entirely** — so the body's gem-level (and life/res) mods don't apply on those builds (proved: removing the body's life mod doesn't change Life; weapon mods *do* apply, so it's body-specific). Only **Vortex** (Galesight helmet) has a live body → it gained the full **+16%**. **The real follow-up this surfaced:** on a chest-forbidding helmet the main 6L should be socketed in the **helmet** (to use the Bringer's 3 free level-30 supports + not waste the void body) — filed as a NEXT STEP.
- **Live generator is the main beneficiary** (per the roadmap): the live `/theorycrafter` builds use rare bases (never Bringer), so every endgame-budget generated body now recommends the two gem-level affixes — a real importable improvement.

**Measured (PoB-exact precompute re-run, all reservation-positive + res-capped + 123 pts):** Vortex 220.9k → **255.8k** (+16%, body live), Arc 109.9k → **139.7k**, Cyclone **173.3k** (=), Lacerate 148.3k → **143.6k** (−3%), Ice Shot **47.3k** (=). The Arc/Lacerate shifts are the deterministic greedy optimiser re-pathing with the richer mod pool (every build remains PoB-validated + legal); the *attributable* gem-level win is Vortex + the live generator. Test: `test_body_armour_has_socketed_gem_level_mods`. Gate: 776 tests / 145 mypy / ruff clean.
...
All phases of the mirror-tier initiative (Steps 58-68) have shipped their levers: precomputed builds now carry **chase uniques** (Step 59) + **layered defences** (Step 60) + **DoT-multiplier gear** for DoT archetypes (Step 61) + **timeless jewels incl. Glorious Vanity** (Steps 63-65) + **reservation-honest multi-aura groups** (Step 67), all trimmed to a **legal level-100 passive-point budget** (Step 68). They went from 12-19% of the top-ladder build to genuinely strong AND playable (Cyclone 187k DPS / 18k EHP, Lacerate 203k / 20k, Vortex 347k / 28k, Arc 192k / 28k, Ice Shot 50k / 19k, plus Boneshatter 53k / Spark 132k / Blade Vortex 28k — every one exactly 123 points, reservation-positive, res-capped, legal socketing, Awakened + corrupted 21/23 gems). The PoB-exact optimiser (local; Render only serves the vendored JSON) is the engine; everything is fitness-gated so nothing can regress a build. **The honest ladder gap + the path to REAL ladder-tier builds is analysed in `docs/REAL_BUILDS_ANALYSIS.md`** — the calc is already PoB-exact, so the work is widening the optimiser's search (Awakened gems ✅ Step 73, then cluster jewels / crafted rares / trigger archetypes).

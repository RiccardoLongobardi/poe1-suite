...
Riccardo did a real **PoB import** of a served Theorycrafter build and found 10 concrete bugs the DPS-only re-eval gate never caught (PoB *warns* about attribute reqs / mana / illegal socketing without zeroing DPS). This step fixes 9 of them; #4 ("imbued support") is left pending a clarification. **The biggest discovery: an importable, castable, ascendancy-allocated build does dramatically more — Vortex 870k → 1.31M, and Boneshatter went from a silent 0 DPS to 635k.** All 8 precomputed builds now re-import into PoB cleanly: castable, ascendancy allocated, legal socketing, real bases, Config tab populated.
...
4. **Imbued (alt-quality) supports** — *pending clarification* (ambiguous; likely Anomalous/Divergent/Phantasmal alt-quality, a marginal Fase-1 lever). Not yet implemented.
...
**Final served builds (PoB-exact, all castable + ascendancy + legal socketing + Ring 2 + Config + ≤123 pts):** Vortex **1,314,742** / EHP 61k, Boneshatter **635,225**, Arc **561,282**, Spark **297,649**, Lacerate **268,630**, Cyclone **222,702**, Ice Shot **70,636**, Blade Vortex **32,114**. Gate: **789 tests / 148 mypy / ruff clean**. **Pending: #4 (alt-quality supports) + a final user PoB re-import to confirm the warnings are gone.**

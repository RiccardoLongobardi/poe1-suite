...
- **Working features (all QA-verified or post-QA fixed)**:
  ...
  - Theorycrafter viability report (Step 43) + connected BFS passive tree path (Step 44) + full-budget tree fill ~120 nodes (Step 45a) + deduplicated gem layout with compatible supports (Step 45b) + 3.28 Awakened gem allowlist (Step 45c) + realistic per-slot item affixes (Step 45d) + localised tree pathing (Step 46) + real item mods via RePoE (Step 47) + tree masteries / clean nodes / honest estimates (Step 48). ✅
...
- **Theorycrafter tree notable-efficiency.** The allocation is connected, localised, weapon-correct and now allocates masteries (Step 48), but it doesn't *optimise* node efficiency — e.g. it may take two separate "+1% to max Fire/Cold Res" points instead of one "+2% to all max elemental Res" notable, or grab redundant small nodes. Real optimisation here is Steiner-tree-ish (maximise build value per point) — a meaningful separate effort. Deferred.

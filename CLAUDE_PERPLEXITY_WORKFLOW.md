...
- **FOB is live in production**, free tier:
  ...
  - Cost: **$0/month**.
- **Baseline gate**: 757 tests green / 137 mypy / ruff clean.
...
**Baseline gate (current): 759 tests green / 138 mypy / ruff clean.**
...
- **Theorycrafter "deep" optimiser — PoB-exact fitness** (initiative, Steps 50+). Goal: generate viable + OP builds from scratch by optimising against PoB's *real* calc, not a keyword proxy. Architecture chosen by Riccardo: hybrid objective (calc + ladder prior), as exact as PoB → run PoB headless; heavy compute lives in (a) offline precompute + (c) a local tool (NOT live on Render).
  ...
  - **Step 55 (NEXT): minion DPS** (the two minion builds calc ~0 DPS — configure the minion as the damage source). **Step 56: gear co-optimisation + precompute + live serving.** Also tracked: chaos-bypasses-ES survivability (chaos+ES with 1 life → tiny chaos EHP; needs Chaos Inoculation allocation or a life pool) and the Shadow life-pool / Saboteur ES-0 edge cases surfaced by the sweep.

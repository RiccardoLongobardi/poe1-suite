...
- **FOB is live in production**, free tier:
  ...
  - Cost: **$0/month**.
- **Baseline gate**: 761 tests green / 138 mypy / ruff clean.
...
**Baseline gate (current): 761 tests green / 138 mypy / ruff clean.**
...
- **Theorycrafter "deep" optimiser — PoB-exact fitness** (initiative, Steps 50+). Goal: generate viable + OP builds from scratch by optimising against PoB's *real* calc, not a keyword proxy. Architecture chosen by Riccardo: hybrid objective (calc + ladder prior), as exact as PoB → run PoB headless; heavy compute lives in (a) offline precompute + (c) a local tool (NOT live on Render).
  ...
  - **Step 56 (NEXT): gear co-optimisation + precompute + live serving.** Defensive follow-ups tracked separately: minion-build *character* pool is low by design (Summon Skeletons ~1900 ES — now LOW_POOL); chaos-bypasses-ES survivability (chaos+ES with 1 life → tiny chaos EHP; needs Chaos Inoculation or a life pool); Shadow life-pool (Frost Blades 744) and Saboteur ES-0 edge cases. **Spectre DPS proper** (vendor monster data + encode a default meta spectre into the export) is its own future item.

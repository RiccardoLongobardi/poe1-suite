...
- **FOB is live in production**, free tier:
  ...
  - Cost: **$0/month**.
- **Baseline gate**: 756 tests green / 137 mypy / ruff clean.
...
**Baseline gate (current): 756 tests green / 137 mypy / ruff clean.**
...
- **Theorycrafter "deep" optimiser — PoB-exact fitness** (initiative, Steps 50+). Goal: generate viable + OP builds from scratch by optimising against PoB's *real* calc, not a keyword proxy. Architecture chosen by Riccardo: hybrid objective (calc + ladder prior), as exact as PoB → run PoB headless; heavy compute lives in (a) offline precompute + (c) a local tool (NOT live on Render).
  ...
  - **Step 52 (NEXT — re-prioritised): GEAR damage mods + gear co-optimisation.** The optimiser proved tree+gems are already good; the real lever is gear. Add flat added-damage / crit mods to `_stat_priorities` + `mods_3_28.json`, then co-optimise gear mods with `PobEvaluator`. This is where realistic / "sbroccate" DPS comes from.
  - **Step 53: precompute pipeline** → vendored per-archetype optima. **Step 54: live serves the vendored optima** (greedy fallback otherwise).

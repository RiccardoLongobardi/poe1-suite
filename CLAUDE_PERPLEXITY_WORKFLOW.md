...
**Baseline gate (current): 757 tests green / 137 mypy / ruff clean.**
...
- **Theorycrafter "deep" optimiser — PoB-exact fitness** (initiative, Steps 50+). Goal: generate viable + OP builds from scratch by optimising against PoB's *real* calc, not a keyword proxy. Architecture chosen by Riccardo: hybrid objective (calc + ladder prior), as exact as PoB → run PoB headless; heavy compute lives in (a) offline precompute + (c) a local tool (NOT live on Render).
  ...
  - **Step 53 (NEXT — re-prioritised by the sweep): elemental-attack classification.** Classify attack-vs-spell by skill **tags**, not damage type, so Lightning Strike / Molten Strike / Frost Blades / Ice Shot get the right weapon (one-hander/bow) + elemental attack damage stats. Biggest correctness cluster.
  - **Step 54: ES pool + chaos res** (ES builds ~3k → boost ES tree/gear; chaos builds undercap res). **Step 55: minion DPS** (configure the minion as the damage source). **Step 56: gear co-optimisation + precompute + live serving.**

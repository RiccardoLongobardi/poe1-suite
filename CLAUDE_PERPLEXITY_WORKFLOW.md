...
- **FOB is live in production**, free tier:
  ...
  - Cost: **$0/month**.
- **Baseline gate**: 754 tests green / 132 mypy / ruff clean.
- **Working features (all QA-verified or post-QA fixed)**:
  ...
  - Theorycrafter viability report (Step 43) + connected BFS passive tree path (Step 44) + full-budget tree fill ~120 nodes (Step 45a) + deduplicated gem layout with compatible supports (Step 45b) + 3.28 Awakened gem allowlist (Step 45c) + realistic per-slot item affixes (Step 45d) + localised tree pathing (Step 46). ✅
...
**Baseline gate (current): 754 tests green / 132 mypy / ruff clean.**
...
- **Theorycrafter real item mods.** Generated rares use *simulated* affix values (`_AFFIX_VALUES`). The user wants real PoE mod tiers/ranges. Requires vendoring the slimmed RePoE mods file (already noted as deferred in CLAUDE.md "Item & Modifier Browser") and rewriting `_theory_item_body` to draw from it. **Next planned step.**

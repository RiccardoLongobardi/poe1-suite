...
- [x] **Bugfix — Trade dialog: unique name search + Instant Buyout + count mods** (2026-05-18) — `/fob/trade-url` was dropping the base type whenever a name was present (name-only query → uniques didn't resolve); now sends `name` + `type` together. Added `TradeQuery.status_option`; `/fob/trade-url` sets it to `"securable"` so prefilled searches default to Instant Buyout. `trade_stats.resolve_mod` gained a `difflib` fuzzy fallback (cutoff 0.9) so count mods GGG stores singular but PoB renders plural (e.g. Mageblood's signature mod) resolve. 713 tests / 121 mypy.

...
def test_theory_generate_e2e(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Step 38 E2E: NL query -> ladder -> hydrated build -> skeleton.

    Real protobuf ladder fixtures + a real captured character JSON are
    replayed via MockTransport. The character endpoint serves the same
    captured build for any account, so whichever ref ranks first always
    resolves to a parseable PoB export.
    """

    # anthropic_api_key=None forces rule-based intent extraction — the
    # LLM fallback must not be exercised in an offline test.
    mirage_settings = Settings(
        cache_dir=tmp_path / "cache",
        http_cache_ttl_seconds=0,
        poe_league="Mirage",
    ...
    def _load_json(name: str) -> dict[str, object]:
        with (BUILDS_FIXTURES / name).open(encoding="utf-8") as f:
    ...
    def mock_handler(request: httpx.Request) -> httpx.Response:
        ...
        if "/builds/" in path and path.endswith("/search"):
            ...
            fx = BUILDS_FIXTURES / f"search_mirage_{klass}.pb"
            if not fx.exists():
                fx = BUILDS_FIXTURES / "search_mirage_all.pb"
            return httpx.Response(
                200,
    ...
    app = create_app(mirage_settings)
    with TestClient(app) as client:
        r = client.post("/fob/theory/generate", json={"query": "build per mapping comodo"})
        assert r.status_code == 200, r.text
        sk = r.json()
        # The captured character is a Marauder Chieftain.
        assert sk["character_class"] == "Marauder"
        assert sk["ascendancy"] == "Chieftain"
        assert sk["main_skill"]
        assert sk["template_name"]
        assert 1 <= sk["level"] <= 100
        assert sk["passive_count"] > 0
        assert "poe.ninja" in sk["source_url"]
        # Honesty clause is always present in the rationale.
        assert "non sono inventati" in sk["rationale"]


def test_theory_generate_rejects_empty_query(settings: Settings) -> None:

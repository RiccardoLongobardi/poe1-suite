...
/** Theorycrafter budget band — distinct from the intent `BudgetTier`. */
export type SkeletonBudget = "starter" | "mid" | "endgame";
export type DefenceArchetype = "life" | "es" | "ward" | "hybrid_life_es";
export type TheoryContentFocus = "mapping" | "bossing" | "allcontent";
...
/** Structured input to the form-driven Build Generator. */
export interface TheoryIntent {
  character_class: string;
  ascendancy: string;
...
export interface TheoryGemLink {
  skill: string;
  supports: string[];
  slot: string;
  label: string;
...
export interface TheoryGearSlot {
  ...
  budget_tier: SkeletonBudget;
}

export interface StatEstimate {
  life_estimate: number;
  es_estimate: number;
  dps_index: number;
...
export interface BuildSkeleton {
  intent: TheoryIntent;
  links: TheoryGemLink[];
  tree_nodes: TreeNodeRef[];
  gear_slots: TheoryGearSlot[];
  stats: StatEstimate;
  rationale_it: string;
  rationale_en: string;
  pob_code: string;
